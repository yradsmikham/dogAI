...
def write_metrics(metrics, global_step, summary_dir):
  ...
  """
  tf._logging.info('Writing metrics to tf summary.')
  ...
  for key in sorted(metrics):
    ...
    tf._logging.info('%s: %f', key, metrics[key])
  tf._logging.info('Metrics written to tf summary.')
...
def visualize_detection_results(result_dict,
  ...
  if show_groundtruth and input_fields.groundtruth_boxes not in result_dict:
    raise ValueError('If show_groundtruth is enabled, result_dict must contain '
                     'groundtruth_boxes.')
  tf._logging.info('Creating detection visualizations.')
  ...
  tf._logging.info('Detection visualizations written to summary with tag %s.',
                  tag)
...
def _run_checkpoint_once(tensor_dict,
  ...
  if save_graph:
    tf.compat.v1.train.write_graph(sess.graph_def, save_graph_dir, 'eval.pbtxt')
  ...
  with slim.queues.QueueRunners(sess):
    try:
      for batch in range(int(num_batches)):
        if (batch + 1) % 100 == 0:
          tf._logging.info('Running eval ops batch %d/%d', batch + 1,
                          num_batches)
        if not batch_processor:
          try:
            ...
            tf._logging.info('Skipping image')
            counters['skipped'] += 1
            result_dict = {}
        else:
      ...
      tf._logging.info('Running eval batches done.')
    except tf.errors.OutOfRangeError:
      tf._logging.info('Done evaluating -- epoch limit reached')
    finally:
      # When done, ask the threads to stop.
      tf._logging.info('# success: %d', counters['success'])
      tf._logging.info('# skipped: %d', counters['skipped'])
      ...
      if eval_export_path and eval_export_path is not None:
        for evaluator in evaluators:
          if (isinstance(evaluator, coco_evaluation.CocoDetectionEvaluator) or
            ...
            tf._logging.info('Started dumping to json file.')
            evaluator.dump_detections_to_json_file(
                json_output_path=eval_export_path)
            tf._logging.info('Finished dumping to json file.')
      for evaluator in evaluators:
...
def repeated_checkpoint_run(tensor_dict,
  ...
  while True:
    start = time.time()
    tf._logging.info('Starting evaluation at ' + time.strftime(
        '%Y-%m-%d-%H:%M:%S', time.gmtime()))
    model_path = tf.train.latest_checkpoint(checkpoint_dirs[0])
    if not model_path:
      tf._logging.info('No model found in %s. Will try again in %d seconds',
                      checkpoint_dirs[0], eval_interval_secs)
    elif model_path == last_evaluated_model_path:
      tf._logging.info('Found already evaluated checkpoint. Will try again in '
...
def result_dict_for_batched_example(images,
  ...
  num_detections = tf.int32(detections[detection_fields.num_detections])

  if class_agnostic:
    detection_classes = tf.ones_like(detection_scores, dtype=tf.int64)
  else:
    detection_classes = (
        tf.int64(detections[detection_fields.detection_classes]) +
        label_id_offset)
